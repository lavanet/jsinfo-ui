...
import { LogpushAxiosGet } from './LogpushAxiosGet';
...
export class LogpushDataFetcher {
    ...
    static initialize(dataKey: string, apiurlDateRangeQuery: any, paginationFetcherHook: any) {
        ...
        const fetcher = new LogpushDataFetcher(apiurlDateRangeQuery, paginationFetcherHook, setData, setLoading, setError, retryTimeout, retryCount, errorCount, isFetching, wasOneFetchDone);
    ...
    async FetchItemCount(updateItemCount: (count: number) => void, retryCount: number = 0): Promise<void> {
        ...
        while (retries < this.maxRetries) {
            try {
                const res = await LogpushAxiosGet("item-count/" + apiurl);
    ...
    async FetchAndPopulateData(): Promise<void> {
        ...
        try {
            ...
            const res = await LogpushAxiosGet(this.apiurl, params);
            const data = res.data;
