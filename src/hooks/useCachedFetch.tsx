...
const handleEmptyData = (retryCount, retryTimeout, fetchDataWithRetryWithRetry, setError, setLoading) => {
    if (retryCount.current < 10) { // If retryCount is less than 10
        // If data is an empty object, retry after retryTimeout milliseconds
        setTimeout(fetchDataWithRetryWithRetry, retryTimeout.current);
...
export function useCachedFetchWithUrlKey(dataKey) {
    ...
    useEffect(() => {
        ...
        if (typeof window !== 'undefined') {
            ...
            fetchDataWithRetryWithRetry(apiUrl, setData, setLoading, setError, retryCount, retryTimeout);
        }
