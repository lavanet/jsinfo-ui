...
const CustomCombobox = ({ availableChains = [], selectedChains = [], onSelectionChange }: { availableChains: string[], selectedChains: string[], onSelectionChange: (updatedSelection: string[]) => void }) => {
    ...
    const selectedChainsForCount = selectedChains.filter(chain => chain !== "All Chains");
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                ...
                >
                    {selectedChainsForCount.length > 0
                        ? `${selectedChainsForCount.length} chain${selectedChainsForCount.length > 1 ? 's' : ''} selected`
                        : "Select chains..."}
